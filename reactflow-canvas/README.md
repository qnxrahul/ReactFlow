...
- Visit `/dynamic` for the new dynamic workflow canvas powered by the backend generator + MCP agents
- Visit `/registry` to register new workflow components/handlers/agents
...
- Registered handlers expose identifiers (e.g., `services.compliance.validate`) that the LLM-driven generator is allowed to reference when drafting workflows. Agents can also be registered here or via `/agents` to link handlers to MCP tools.
